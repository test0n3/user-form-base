class User < ApplicationRecord
  ...
  validates :email, uniqueness: true, confirmation: true, format: { with: /\A[^@\s]+@([^@\s]+\.)+[^@\s]+\z/ }
  # validates :email_confirmation, presence: true
  validates :password, confirmation: true, length: { minimum: 8 }, format: { with: /\A(?=(.*[a-z]){1,})(?=(.*[\d]){1,})(?!.*\s).{8,20}\Z/}
